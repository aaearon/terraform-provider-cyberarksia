...
| `vpc_ids` | `vpc-` + 8 or 17 alphanumeric chars | `vpc-12345678`, `vpc-1234567890abcdef0` |
...
| Field | Format | Example |
|-------|--------|---------|
| `projects` | 3-60 chars, lowercase letters/numbers/hyphens, starts with letter | `my-project-123` |
...
VM policies can be imported using the policy ID:

```bash
terraform import cyberarksia_vm_policy.example <policy_id>
```

**Note**: All principals assigned to the policy at import time will become inline principals in your Terraform state. For brownfield adoption where principals are managed externally, consider using `cyberarksia_vm_policy_principal_assignment` resources after import.

## Additional Examples

For cloud-specific and RDP configurations, see:

- [AWS Policy](../../examples/resources/cyberarksia_vm_policy/aws_policy.tf) - VPCs, regions, account IDs, tags
- [Azure Policy](../../examples/resources/cyberarksia_vm_policy/azure_policy.tf) - VNets, subscriptions, resource groups
...
|-------|-------|------------|
| HTTP 500 on Azure policy | SDK casing bug (automatic workaround applied) | Ensure you're using latest provider version |
