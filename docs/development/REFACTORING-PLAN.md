...
- `policy_workspace_assignment_resource.go`: 1,177 → 767 lines (-410 lines / 35% reduction)
...
- `policy_workspace_assignment_resource.go` reduces from 1,177 to ~400 LOC
...
The file `internal/provider/policy_workspace_assignment_resource.go` (1,177 lines) contains **4 identical 200+ line switch statements** that handle 6 authentication profiles:
...
**Impact**: Reduce `policy_workspace_assignment_resource.go` from 1,177 to ~400 LOC
...
**File to Modify**: `internal/provider/policy_workspace_assignment_resource.go`
...
# Check line counts
wc -l internal/provider/policy_workspace_assignment_resource.go
...
**Success Criteria**:
- ✅ All tests pass
- ✅ `policy_workspace_assignment_resource.go` is under 500 LOC
...
**Files to Modify**:
1. `internal/provider/policy_workspace_assignment_resource.go`
2. `internal/provider/database_workspace_resource.go`
3. `internal/provider/database_policy_resource.go`

**Example Refactoring** (policy_workspace_assignment_resource.go):
...
```bash
# In policy_workspace_assignment_resource.go
...
2. `internal/provider/policy_workspace_assignment_resource.go` (lines ~238, ~288)
...
### Success Metrics
- [ ] All tests pass
- [ ] `policy_workspace_assignment_resource.go` < 500 LOC
...
- Main refactor target: `internal/provider/policy_workspace_assignment_resource.go`
